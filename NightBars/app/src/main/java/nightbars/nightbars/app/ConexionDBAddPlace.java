...
public class ConexionDBAddPlace extends AsyncTask<String, Integer, Integer> {
    private Context context;
    private TaskCallback mCallback;
    private final ProgressDialog progressDialog;

    public ConexionDBAddPlace(Context context, TaskCallback callback) {
    ...
    protected void onPostExecute(Integer result) {
        if (result != 0) {
            Toast.makeText(context, "Añadido con éxito!", Toast.LENGTH_LONG).show();
        } else {
            Toast.makeText(context, "Se ha producido un error al añadir.", Toast.LENGTH_LONG).show();
        }
